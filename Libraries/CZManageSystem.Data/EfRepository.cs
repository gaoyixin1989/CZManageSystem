    ...
    public partial class EfRepository<T> : IRepository<T> where T : class
    {
        ...
        public virtual bool ExecuteSqlCommand(string sql)
        {
            try
            {
                ...
                return this._context.ExecuteSqlCommand(sql) > 0;
        ...
        /// 执行SQL
        /// </summary> 
        /// <param name="sqlQuery">SSQL</param>
        /// <param name="parameters">参数</param>
        /// <returns></returns>
        public async Task<int> ExecuteSqlCommandAsync(string sqlQuery, params object[] parameters)
        {
            try
            {
                var slt = this._context.Database.ExecuteSqlCommandAsync(sqlQuery, parameters);
                return await slt;//.ToList(); 
            }
            catch (DbEntityValidationException dbEx)
            {
                return 0;
