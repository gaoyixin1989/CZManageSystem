...
namespace CZManageSystem.Core
{
    ...
    public class LogRecord
    {
        ...
        public static void WriteCanteenServiceLog(string strMsg, string strResult)
        {
            checkFile();
            string logName = "Canteen" + DateTime.Now.ToString("yyyy-MM-dd") + ".txt";
            try
            {
                string strDateTime = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
                string strData = "\r\n" + strDateTime + "\t\t" + strResult + "\r\n"  + "Detail : " + strMsg + ((char)13) + ((char)10) ;
                FileStream fs = new FileStream(strPath + logName, FileMode.Append, FileAccess.Write);
                StreamWriter sw = new StreamWriter(fs, Encoding.UTF8);
                sw.WriteLine(strData);
