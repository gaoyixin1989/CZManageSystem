...
{
    public class PayVService : BaseService<PayV>, IPayVService
    {
        static SystemContext dbContext = new SystemContext("SqlConnectionHR");
        public PayVService() : base(dbContext)
        { }
        /// <summary>
        /// 分页
        /// </summary>
        ...
        public override IEnumerable<dynamic> GetForPaging(out int count, object objs = null, int pageIndex = 0, int pageSize = int.MaxValue)
        {
            try
            {
                ...
                PagedList<PayV> pageList = new PagedList<PayV>(source, pageIndex <= 0 ? 0 : (pageIndex - 1), pageSize, out count);
