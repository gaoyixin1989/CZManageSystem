...
{
    public class HRAttendanceConfigService : BaseService<HRAttendanceConfig>, IHRAttendanceConfigService
    {
        ...
        public override IEnumerable<dynamic> GetForPaging(out int count, object objs = null, int pageIndex = 0, int pageSize = int.MaxValue)
        {
            try
            {
                ...
                PagedList<HRAttendanceConfig> pageList = new PagedList<HRAttendanceConfig>(source.Where (w=>!w.DeptIds.Contains("NULL")), pageIndex <= 0 ? 0 : (pageIndex - 1), pageSize, out count);
