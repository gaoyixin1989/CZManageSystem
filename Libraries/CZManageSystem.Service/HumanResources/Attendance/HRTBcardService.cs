...
{
    public class HRTBcardService : BaseService<HRTBcard>, IHRTBcardService
    {
        ...
        public override IEnumerable<dynamic> GetForPaging(out int count, object objs = null, int pageIndex = 0, int pageSize = int.MaxValue)
        {
            try
            {
                ...
                PagedList<HRTBcard> pageList = new PagedList<HRTBcard>(source, pageIndex <= 0 ? 0 : (pageIndex - 1), pageSize, out count);
