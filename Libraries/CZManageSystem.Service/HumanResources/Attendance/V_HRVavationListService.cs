...
{
    public class V_HRVavationListService : BaseService<V_HRVavationList>, IV_HRVavationListService
    {
        ...
        public override IEnumerable<dynamic> GetForPaging(out int count, object objs = null, int pageIndex = 0, int pageSize = int.MaxValue)
        {
            try
            {
                var source = objs == null ? this._entityStore.Table.OrderByDescending(c => c.DoTime) : this._entityStore.Table.OrderByDescending(c => c.DoTime).Where(ExpressionFactory(objs));
                PagedList<V_HRVavationList> pageList = new PagedList<V_HRVavationList>(source, pageIndex <= 0 ? 0 : (pageIndex - 1), pageSize, out count);
