...
{
    public class HRUnattendLinkService : BaseService<HRUnattendLink>, IHRUnattendLinkService
    {
        ...
        public override IEnumerable<dynamic> GetForPaging(out int count, object objs = null, int pageIndex = 0, int pageSize = int.MaxValue)
        {
            try
            {
                ...
                PagedList<HRUnattendLink> pageList = new PagedList<HRUnattendLink>(source, pageIndex <= 0 ? 0 : (pageIndex - 1), pageSize, out count);
