...
{
    public class V_HROtherListService : BaseService<V_HROtherList>, IV_HROtherListService
    {
        ...
        public override IEnumerable<dynamic> GetForPaging(out int count, object objs = null, int pageIndex = 0, int pageSize = int.MaxValue)
        {
            try
            {
                ...
                PagedList<V_HROtherList> pageList = new PagedList<V_HROtherList>(source, pageIndex <= 0 ? 0 : (pageIndex - 1), pageSize, out count);
