...
{
    public class HRVacationCoursesService : BaseService<HRVacationCourses>, IHRVacationCoursesService
    {
        static Users _user;
        static List<HRVacationCourses> listDic = new List<HRVacationCourses>();
        ...
        public IList<HRVacationCourses> GetForPagingByCondition(Users user, out int count, HRVacationCoursesQueryBuilder objs = null, int pageIndex = 0, int pageSize = int.MaxValue)
        {
            ...
            if (objs.ProvinceCity != null && objs.ProvinceCity != "")
                curData = curData.Where(u => objs.ProvinceCity.Contains(u.ProvinceCity));
            if (objs.EmployeeID != null && objs.EmployeeID != "")
                curData = curData.Where(u => u.EmployeeId.Contains(objs.EmployeeID));
            if (objs.CoursesType != null && objs.CoursesType != "")
                curData = curData.Where(u => objs.CoursesType.Contains(u.CoursesType));
            ...
            count = curData.Count();
            var list = curData.OrderByDescending(p => p.StartTime).Skip(pageIndex * pageSize).Take(pageSize).AsEnumerable().Select(x => new HRVacationCourses()
            {
                CoursesId = x.CoursesId,
                CoursesName = x.CoursesName,
                CoursesType = x.CoursesType,
                UserId = x.UserId,
        ...
        public IList<HRVacationCourses> GetForPersonalPagingByCondition(out int count, HRVacationCoursesQueryBuilder objs = null, int pageIndex = 0, int pageSize = int.MaxValue)
        {
            var curData = from hrvc in this._entityStore.Table
                          join bwu in this._bw_Users.Table
                          on hrvc.UserId equals bwu.UserId
        ...
        public dynamic ImportVacationCourses(string filename,Stream fileStream, Users user)
        {
            try
            {
                ...
                if (dataTable.Rows.Count < 1)
                    return false;
                listDic = this._entityStore.Table.ToList() as List<HRVacationCourses>;
                listDicUsers = new EfRepository<Users>().Table.ToList() as List<Users>;
                List<HRVacationCourses> list = new List<HRVacationCourses>();
        ...
        HRVacationCourses GetModel(DataRow dataRow, out List<string> tip,out HRIntegral HRIntegralModel,out string shtmw)
        {
            //验证数据是否合法 
            tip = new List<string>();
            shtmw = "";
            ...
            HRVacationCourses temp = new HRVacationCourses();
            ...
            else
            {
                ...
                var tmp3 = Convert.ToDateTime(_temp[5]);
                var checkexsitlist = CurrDataBase.Where(u => u.CoursesName == tmp1).Where(u=>u.UserName== tmp1).Where(u=>u.StartTime.Value == tmp3).ToList();
                ...
                }else
                {
                    ...
                    else
                    {
                        ...
                        //检测天数的基础配置
                        var configlist = _hrcintegralconfigservice.JudgeConfig(Convert.ToDouble(_temp[7].ToString()));
            ...
            if (buseformula == "公式")
                c_integral = Convert.ToDouble(_temp[7].ToString()) * times;
            else if (buseformula == "常量")
                c_integral = integral;

            temp.CoursesId = Guid.NewGuid();
            temp.Integral = Convert.ToDecimal(c_integral);
            temp.CoursesName = _temp[2].ToString();
            temp.CoursesType = _temp[3].ToString();
            temp.ProvinceCity = _temp[4].ToString();
            temp.StartTime = string.IsNullOrEmpty(_temp[5].ToString()) ? TimeNull : Convert.ToDateTime(_temp[5].ToString());
            temp.EndTime = string.IsNullOrEmpty(_temp[6].ToString()) ? TimeNull : Convert.ToDateTime(_temp[6].ToString());
            temp.PeriodTime = string.IsNullOrEmpty(_temp[7]?.ToString()) ? decimalnull : Convert.ToDecimal(_temp[7].ToString());
            temp.AgreeFlag = 2;
            ...
            HRIntegralModel.Daoid = temp.CoursesId;
