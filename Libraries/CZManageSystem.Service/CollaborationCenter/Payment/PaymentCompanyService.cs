...
{
    public partial class PaymentCompanyService : BaseService<PaymentCompany>, IPaymentCompanyService
    {
        /// <summary>
        /// 分页
        /// </summary>
        ...
        {
            var source = objs == null ? this._entityStore.Table.OrderBy(c => c.DpId) : this._entityStore.Table.OrderBy(c => c.DpId).Where(ExpressionFactory(objs));
            PagedList<PaymentCompany> pageList = new PagedList<PaymentCompany>(source, pageIndex <= 0 ? 0 : (pageIndex - 1), pageSize, out count);
            count = pageList.TotalCount;
            return pageList.Select(s => new
            {
                s.DpId,
                s.DpName,
                s.PaymentPayers.FirstOrDefault()?.Account
            });
        }
    }
}
