...
{
    public partial class PendingJobService : BaseService<IAMS_PendingJob>, IPendingJobService
    {
        /// <summary>
        /// 待办工作分页
        /// </summary>
        ...
        public override IEnumerable<dynamic> GetForPaging(out int count, object objs = null, int pageIndex = 0, int pageSize = int.MaxValue)
        {
            ...
            return new PagedList<IAMS_PendingJob>(curTable.OrderBy(p=>p.ProcessedDT).Skip(0).Take(100), pageIndex <= 0 ? 0 : (pageIndex - 1), pageSize, out count);
        }
    }
}
