...
{
   public class PersonalWelfareManageMonthInfoService : BaseService<PersonalWelfareManageMonthInfo>, IPersonalWelfareManageMonthInfoService
    {
        static Users _user;
        ...
        public dynamic Import(Stream fileStream, Users user)
        {
            try
            {
                ...
                List<PersonalWelfareManageMonthInfo> list = new List<PersonalWelfareManageMonthInfo>();
        ...
        PersonalWelfareManageMonthInfo GetModel(DataRow dataRow, out List<string> tip)
        {
            tip = new List<string>();
            var _temp = dataRow.ItemArray;
            PersonalWelfareManageMonthInfo temp = new PersonalWelfareManageMonthInfo();
            ...
            if (string.IsNullOrEmpty(_temp[2]?.ToString().Trim()))
                ...
                temp.CYearAndMonth = _temp[2]?.ToString().Trim();
            if (string.IsNullOrEmpty(_temp[3]?.ToString().Trim()))
                tip.Add("福利套餐不能为空");
            else
                temp.WelfarePackage = _temp[3]?.ToString().Trim();
            Decimal TotalAmount = 0;
            if (string.IsNullOrEmpty(_temp[4]?.ToString().Trim()))
                tip.Add("总额度不能为空");
            else if (!Decimal.TryParse(_temp[4]?.ToString().Trim(), out TotalAmount))
                tip.Add("数量为小数类型");
            else
                temp.WelfareMonthTotalAmount = TotalAmount;

            Decimal AmountUsed = 0;
            if (string.IsNullOrEmpty(_temp[5]?.ToString().Trim()))
                tip.Add("已用额度不能为空");
            else if (!Decimal.TryParse(_temp[5]?.ToString().Trim(), out AmountUsed))
                tip.Add("数量为小数类型");
            else
                temp.WelfareMonthAmountUsed = AmountUsed;

            //temp.Editor = _user.UserName;
            temp.CreateTime = DateTime.Now;
            temp.EditTime = DateTime.Now;
            temp.MID = Guid.NewGuid();
            if (tip.Count > 0)
                return null;
            return temp;
        }
