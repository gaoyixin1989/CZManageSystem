...
namespace Botwave.Workflow.Practices.CZMCC.Support
{
    ...
    /// 带抄送功能的销售精英申告流程选择器类.
    /// </summary>
    public class ExtendXSJYWorkflowSelectorProfile : Botwave.XQP.Service.Support.ExtendWorkflowSelectorProfile
    {
        /// <summary>
        /// 个性化的流程活动(步骤)名称.
        /// </summary>
        ...
        #region IWorkflowSelectorProfile interface
            ...
            else
            {
                ...
                foreach (ActivityDefinition next in selecotrContext.NextActivities)
                {
                    if (next.ActivityName.Equals(_profileActivityName, StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        if (workflowReviewType == ReviewType.CheckBox)
                        {
                            return this.XSJYBuildExtendActivitySelectorHtmlByCheck(webContext, selecotrContext, workflowReviewType);
                        }
                        else if (workflowReviewType == ReviewType.Classic)
                        {
                            return this.XSJYBuildExtendActivitySelectorHtmlByClassic(webContext, selecotrContext);
                        }
                        return this.XSJYBuildActivitySelectorHtml(webContext, selecotrContext);
                    }
                }
            }
            return base.BuildActivitySelectorHtml(webContext, selecotrContext, workflowReviewType);
        }
        #endregion
        ...
        protected virtual string XSJYBuildDefaultRadioActivities(IList<WorkflowSelectorContext.ActivityActor> activityActors, Guid specialActivityId, string activityItemTemplate, string actorItemTemplate)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (item.ActivityId == specialActivityId)
                {
                    ...
                    //    keyIndex++;
                    //}
                    actorsBuilder.AppendLine(ExtendWorkflowSelectorProfile.BuildPopupButton());
                    actorsBuilder.AppendLine("</div>");

                    actorsBuilder.AppendLine(ExtendWorkflowSelectorProfile.BuildPopupScript(item.ActivityId, i));
                    continue;
        ...
        /// <param name="selecotrContext"></param>
        /// <returns></returns>
        protected virtual string XSJYBuildExtendActivitySelectorHtmlByClassic(HttpContext webContext, WorkflowSelectorContext selecotrContext)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append(this.XSJYBuildActivitySelectorHtml(webContext, selecotrContext));
        ...
        /// <returns></returns>
        protected virtual string XSJYBuildExtendActivitySelectorHtmlByCheck(HttpContext webContext, WorkflowSelectorContext selecotrContext, ReviewType reviewType)
        {
            ...
            {
                activityItemTemplate = "<input type=\"checkbox\" id=\"activityOption_{0}\" name=\"activityOption\" value=\"{1}\" activity=\"{2}\" onclick=\"toggleCheckBoxAllocator('activityAllocatee{3}',this,'actors_{0}',{5});\" {4} /><span{6}>{2}</span>";
                return this.XSJYBuildExtendCheckBoxActivities(activityActors, this.activityProfiles, reviewType, specileActivityId, selectAll, activityItemTemplate, Template_ActorInputHtml);
            }
            else
            {
                activityItemTemplate = "<input type=\"radio\" id=\"activityOption_{0}\" name=\"activityOption\" value=\"{1}\" activity=\"{2}\" onclick=\"toggleRadioAllocator('activityAllocatee{3}', this, 'actors_{0}',{5});\" {4} /><span{6}>{2}</span>";
                return this.XSJYBuildExtendRadioActivities(activityActors, this.activityProfiles, reviewType, specileActivityId, activityItemTemplate, Template_ActorInputHtml);
            }
        }
        ...
        /// <param name="actorItemTemplate"></param>
        /// <returns></returns>
        protected virtual string XSJYBuildExtendRadioActivities(IList<WorkflowSelectorContext.ActivityActor> activityActors, IDictionary<Guid, ActivityProfile> reviewProfiles, ReviewType reviewType, Guid specialActivityId, string activityItemTemplate, string actorItemTemplate)
        {
            StringBuilder htmlBuilder = new StringBuilder();
            StringBuilder actorsBuilder = new StringBuilder();
            int count = activityActors.Count;
        ...
        /// <param name="actorItemTemplate"></param>
        /// <returns></returns>
        protected virtual string XSJYBuildExtendCheckBoxActivities(IList<WorkflowSelectorContext.ActivityActor> activityActors, IDictionary<Guid, ActivityProfile> reviewProfiles, ReviewType reviewType, Guid specialActivityId, bool selectAll, string activityItemTemplate, string actorItemTemplate)
        {
            StringBuilder htmlBuilder = new StringBuilder();
            int count = activityActors.Count;
            bool isChecked = (count <= SelectedBound || selectAll);
