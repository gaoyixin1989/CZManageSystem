...
namespace Botwave.Workflow.Practices.CZMCC.Support
{
    ...
    /// 特殊需求流程选择器类.
    /// </summary>
    public class WorkflowSelectorProfile : Botwave.Workflow.Extension.UI.Support.DefaultWorkflowSelectorProfile
    {
        #region properties
        ...
        public override string BuildActivitySelectorHtml(HttpContext webContext, WorkflowSelectorContext selecotrContext)
        {
            ...
            {
                //actor = actor.Trim().ToLower();  // 当前处理人.
                //string creator = selecotrContext.WorkflowInstance.Creator; // 发单人(申请人).
                //IList<string> todoActors = GetTodoActors(selecotrContext.ActivityInstanceId);

                //if (!actor.Equals(creator, StringComparison.CurrentCultureIgnoreCase)
                //    || todoActors.Count != 1 || !todoActors[0].Equals(actor, StringComparison.CurrentCultureIgnoreCase))
                //{
                //    // 在指定步骤实行特定流程步骤选择器.
                //    return this.CZBuildActivitySelectorHtml(webContext, selecotrContext);
                //}
                return CZBuildDefaultActivitySelectorHtml(webContext, selecotrContext);
            }
