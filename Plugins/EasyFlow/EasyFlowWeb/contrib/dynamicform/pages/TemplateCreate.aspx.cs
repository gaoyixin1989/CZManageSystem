...
public partial class contrib_dynamicform_pages_TemplateCreate : Botwave.Security.Web.PageBase
{
    ...
    private IDataFormLibraryService dataFormLibraryService = (IDataFormLibraryService)Ctx.GetObject("dataFormLibraryService");
    ...
    protected override void OnInit(EventArgs e)
    {
        ...
        if (!string.IsNullOrEmpty(fdid))
        {
            ...
            if (null != temp)
            {
                ...
                txtFormName.Text = temp.Name;
                txtFormRemark.Text = temp.Comment;
            }
        }
        else
        {
    ...
    protected void btnCreateTemplate_Click(object sender, EventArgs e)
    {
        ...
        dataFormLibraryService.UpdateFormDefinition(definition);

        SqlParameter[] pa = new SqlParameter[2];
        pa[0] = new SqlParameter("@formDefinitionId", SqlDbType.UniqueIdentifier);
        pa[0].Value = FormDefinitionId;
        pa[1] = new SqlParameter("@WapTemplateContent", SqlDbType.Text);
        pa[1].Value = WapTemplateContent;
        object result = IBatisDbHelper.ExecuteScalar(CommandType.Text, "update bwdf_FormDefinitions set WapTemplateContent=@WapTemplateContent where id=@formDefinitionId ", pa);
        /*Guid returnId = FormDefinitionId;
        if (Guid.Empty != returnId)
        {
            ...
            formDefinitionService.RemoveItemsFromForm(itemList);
            ...
            itemDataListSettingService.DataListSettingDelete(FormDefinitionId);
            foreach (DataListSetting setting in settings)
            {
                ...
                dataListDefinitionService.RemoveItemsFromForm(itemDefinitions);
            ...
            foreach (FormItemExtension item in extensionlist)
            {
                ...
                //item.FormDefinitionId = returnId;
                getDataService.DeleteFormItemExtensionById(item.FormItemDefinitionId);//先删除原有的设置
            ...
            foreach (FormItemIFrames item in iFrames)
            {
                if (idList.ContainsKey(item.FormItemDefinitionId.ToString()))
                    item.FormItemDefinitionId = new Guid(idList[item.FormItemDefinitionId.ToString()]);
                //item.FormDefinitionId = returnId;
                formItemIFramesService.Delete(item.FormItemDefinitionId);//先删除原有配置
                formItemIFramesService.Create(item);
            }

            
        }*/
        ShowSuccess("创建模板成功", this.Request.RawUrl);
        //ShowError("创建模板失败!");
    }
    ...
    {
        FormDefinitionsInExternals external = new FormDefinitionsInExternals();
            external.FormDefinitionId = FormDefinitionId;
            external.EntityType = "Form_Template";//修改
            external.EntityId = Guid.Empty;
            if (formDefinitionService.GetFormDefinitionByExternalEntity(external.EntityType,external.EntityId)==null)
