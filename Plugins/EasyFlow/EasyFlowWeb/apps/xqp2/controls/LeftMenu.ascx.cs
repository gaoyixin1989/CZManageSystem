...
public partial class apps_xqp2_extension_controls_LeftMenu : Botwave.Security.Web.UserControlBase
{
        ...
        IDictionary<string, string> userResources)
    {
        ...
        foreach (WorkflowMenuGroup item in groups)
        {
            ...
            builder.AppendLine(string.Format("<li><h4 title=\"{1}\">{0}</h4>", groupName, item.GroupName));
    ...
    {
        StringBuilder builder = new StringBuilder();
        builder.AppendLine("<div class=\"menuContent\">");
        workflowCount = 0;
