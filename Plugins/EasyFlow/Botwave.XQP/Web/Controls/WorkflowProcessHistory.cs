...
namespace Botwave.XQP.Web.Controls
{
    ...
    /// 流程处理历史信息.
    /// </summary>
    public abstract class WorkflowProcessHistory : WorkflowControlBase
    {
        /// <summary>
        /// 转交表顶部.
        /// </summary>
        protected const string AssignmentHeader = "<th>转交人</th><th>被转交人</th><th>转交时间</th><th>转交信息</th>";
        /// <summary>
        /// 会签表顶部.
        /// </summary>
        protected const string CountersignedHeader = "<th colspan='2'>会签人</th><th>会签时间</th><th>会签信息</th>";
        ...
        {
            get { return ViewState["IsToShortDateString"] == null ? false : (bool)(ViewState["IsToShortDateString"]); }
            set { ViewState["IsToShortDateString"] = value; }
        }
        #endregion
        ...
        public virtual void Initialize(string workflowName, Guid workflowInstanceId)
        {
            ...
            if (this.Visible)
            {
                ...
                WorkflowProfile profile = Botwave.Extension.IBatisNet.IBatisMapper.Load<WorkflowProfile>("xqp_WorkflowSetting_Select_ByWorkflowInstanceId", workflowInstanceId);
                if (profile != null)
                    this.IsToShortDateString = profile.IsToShortDateString;
        ...
        protected virtual string BuildActivityCells(CZActivityInstance item, string currentActor)
        {
            ...
            // 执行时间
            if(this.IsToShortDateString)
                builder.Append(item.IsCompleted ? string.Format("<td>{0:yyyy-MM-dd}</td>", item.FinishedTime) : "<td></td>");
            else
            ...
            // 处理意见
            builder.AppendFormat("<td>{0}</td>", reason);
        ...
        protected virtual string BuildActivityAssginmentCells(Assignment item)
        {
            ...
            // 执行时间
            if (this.IsToShortDateString)
                builder.AppendFormat("<td>{0:yyyy-MM-dd}</td>", item.AssignedTime);  // 转交时间.
            else
                builder.AppendFormat("<td>{0:yyyy-MM-dd HH:mm:ss}</td>", item.AssignedTime);  // 转交时间.
            builder.AppendFormat("<td>{0}</td>", item.Message);    // 转交信息
            builder.Append("</tr>");
            return builder.ToString();
        }
        ...
        protected virtual string BuildActivityCountersignedCells(Countersigned item, bool rowStartTag)
        {
            ...
            builder.AppendFormat("<span tooltip=\"{0}\"><b>{1}</b></spa>", user.UserName, user.RealName);
            builder.Append("</td>");
            // 执行时间
            if (this.IsToShortDateString)
                builder.AppendFormat("<td>{0:yyyy-MM-dd}</td>", item.CreatedTime);    // 会签时间
            else
                builder.AppendFormat("<td>{0:yyyy-MM-dd HH:mm:ss}</td>", item.CreatedTime);    // 会签时间
            builder.AppendFormat("<td>{0}</td>", item.Message);    //  会签信息
