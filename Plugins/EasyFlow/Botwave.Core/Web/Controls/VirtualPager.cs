...
namespace Botwave.Web.Controls
{
    ...
    public class VirtualPager : WebControl, INamingContainer
    {
        ...
        private void BuildControlHierarchy()
        {
            ...
            if (PagerStyle != PagerStyle.NumericPages)//为修改BuildNumericPagesUI，gyx添加此判断
            {
        ...
        private void BuildNumericPagesUI(TableCell cell)
        {
            ...
            //'<span>共 <b>{{conf.currentPage+"/"+conf.numberOfPages}}</b> 页 第<input type="text" ng-model="jumpPageNum"/>页</span>'

            //跳转输入框
            cell.Controls.Add(new LiteralControl(string.Format("<div class='paging'><span>共 <b>{0}/{1}</b> 页 第", (CurrentPageIndex + 1), TotalPages)));
            TextBox goToPageTxt = new TextBox();
            goToPageTxt.ID = "txtGoToPage";
            goToPageTxt.Font.Size = Font.Size;
            goToPageTxt.Width = 25;
