...
namespace Botwave.DynamicForm.Renders
{
    ...
    public class ControlCreator
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static Table CreateTableSchema()
        {
            Table tbBody = new Table();
            tbBody.ID = "TB_FORM_DATA";
            tbBody.CssClass = "tblGrayClass grayBackTable";
            tbBody.CellPadding = 4;
            tbBody.CellSpacing = 1;
            tbBody.Style.Value = "word-break:break-all";
            return tbBody;
        }
        ...
        /// </summary>
        /// <returns></returns>
        public static TableRow CreateSingleRow()
        {
        ...
        /// </summary>
        /// <returns></returns>
        public static TableRow CreateDoubleRow()
        {
            TableRow tr = new TableRow();
            /* 用了 <th> 标签显示标题背景色，就不显示行背景色.  
             * 如果需要改回"<td>"标签，择需改动 
             * 1."ControlCreator.CreateTitleCell(string title)";
             * 2."TableRender.AppendRowNumber(Table tbForm, bool appendRowNumber)".
            */
            //tr.CssClass = "trClass";
            return tr;
        }
        ...
        /// </summary>
        /// <returns></returns>
        public static TableCell CreateTitleCell()
        {
        ...
        {
            sw.Write(String.Format("<input type=\"text\" id=\"{0}\" name=\"{0}\" ", CONTROL_PREFIX + FormItemDefinition.FormItemPrefix.Text + item.FName));
        ...
        {
            sw.Write(String.Format("<textarea class=\"MultiLine\" id=\"{0}\" name=\"{0}\" ", CONTROL_PREFIX + FormItemDefinition.FormItemPrefix.TextArea + item.FName));
            sw.Write("onkeyup=\"adjustTextArea(this)\" ");
        ...
        {
            sw.Write(String.Format("<input name=\"{0}\" id=\"{0}\" value=\"$!tc.GetValue(&quot;{1}&quot;)\" ", CONTROL_PREFIX + FormItemDefinition.FormItemPrefix.Date + item.FName, item.FName));
            sw.Write("type=\"text\" class=\"inputbox\" style=\"width:80px;\"");
            AppendValidateAttributes(sw, validate);
            sw.Write(" /><span class=\"ico_pickdate\" title=\"点击选择日期\" onclick='return showCalendar(&quot;");
            sw.Write(CONTROL_PREFIX + FormItemDefinition.FormItemPrefix.Date + item.FName + "&quot;, &quot;%Y-%m-%d&quot;, &quot;24&quot;, true);'>&nbsp;</span>");
        }
        ...
        public static void RenderSelect(StringWriter sw, FormItemDefinition item, FormItemDefinition.ValidateTemplate validate)
        {
            ...
            sw.Write(String.Format("<select id=\"{0}\" name=\"{0}\" class=\"{1}\"", CONTROL_PREFIX + FormItemDefinition.FormItemPrefix.DropDownList + item.FName, "form_ddl"));
        ...
        {
            sw.Write("<div id=\"divHTMLContainer\">");
            sw.Write(String.Format("$!tc.GetValue(&quot;{0}&quot;)", item.FName));
