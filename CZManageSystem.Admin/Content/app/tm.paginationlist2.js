...
angular.module('tm.pagination', []).directive('tmPagination', [function () {
    return {
        ...
        template: ' <div class="paging">' +
              '<div class="no-items" ng-show="conf.totalItems <= 0" style="line-height:23px;">暂无数据</div>' +
            '<div>' +
              //'<a   class="current"  ng-show="conf.totalItems>0" >◀</a>' + 
              '<a ng-class="{disabled: conf.currentPage == 1}" ng-click="prevPage()""><</a>' +
             ' <a ng-repeat="item in pageList track by $index" ng-class="{current:item==conf.currentPage, active: item == conf.currentPage, separate: item == \'...\'}" ' +
              'ng-click="changeCurrentPage(item)">{{ item }}</a>' +
              '<a ng-class="{disabled: conf.currentPage == conf.numberOfPages}" ng-click="nextPage()"">></a>' +
              '<span>转到第<input type="text" ng-model="jumpPageNum"/>页</span>' +
                '<a  ng-click="jumpToPage($event)">GO</a>' +
                '</div>' +
             //' <a  class="page-total" ng-show="conf.totalItems > 0">▶</a>'  +
              '  </div>',



        /*template: '<div class="paging">' +
          '<ul class="current" ng-show="conf.totalItems > 0">' +
        ...
        link: function (scope, element, attrs) {
            ...
            scope.changeCurrentPage = function (item) {
                if (item == '...') {
                    ...
                    //ele.parent("div").children("a").not(ele).removeClass("current");
                }
            };
