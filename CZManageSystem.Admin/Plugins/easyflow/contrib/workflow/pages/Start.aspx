<%@ Page Language="C#" MasterPageFile="~/plugins/easyflow/masters/Default.master" AutoEventWireup="true" Inherits="contrib_workflow_pages_Start" Codebehind="Start.aspx.cs" %>
    ...
    <div class="dataList">
        ...
        <div class="dataTable" id="divAttachmentContainer">
            <iframe id="ifAttachment" src="<%=AppPath%>contrib/workflow/pages/WorkflowAttachment.aspx?wid=<%=WorkflowId %>&&wiid=<%=WorkflowAttachmentId %>" scrolling="no"
                frameborder="no" width="100%" onload="this.height=$(this).contents().find('body').height()+20">
            </iframe>
        </div>
...
//        });
        function beforeCreate() {
            var regx = /^[^\|\"\'\<\>]/g;
            var title = $("#ctl00_cphBody_myWorkItemInput_txtTitle").val();
        ...
        var FormDataSet = getFormDataSet("<%=WorkflowName %>", "<%=WorkflowAttachmentId %>", $("#ctl00_cphBody_myWorkItemInput_txtTitle").val(), "", "<%=CurrentUserName %>", "<%=DpId %>", "", "<%=ActivityName %>");
