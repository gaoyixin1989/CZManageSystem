<%@ Page Language="C#" MasterPageFile="~/plugins/easyflow/masters/Default.master" AutoEventWireup="true" Inherits="contrib_workflow_pages_WorkflowIndex" Codebehind="WorkflowIndex.aspx.cs" %>
<%@ Register TagPrefix="bw" TagName="TaskList" Src="../controls/WorkflowTaskList.ascx"%>
...
<asp:Content ID="Content2" ContentPlaceHolderID="cphBody" Runat="Server">
    ...
    function onStartWorkflow(){
        location = "start.aspx?wid=<%=this.WorkflowId%>";
    }
    var hasLoadedDoneTasks = false;
    function onShowDoneTasks(obj){
        if (!hasLoadedDoneTasks){
            $("#frameDoneTask").attr("src","WorkflowDoneIndex.aspx?wname=<%=HttpUtility.UrlEncode(this.WorkflowName)%>");
    ...
    function onShowStat(obj){
        if (!hasLoadedStat){
            $("#frameWorkflowStat").attr("src","stat/WorkflowStat.aspx?wname=<%=HttpUtility.UrlEncode(this.WorkflowName)%>");
            hasLoadedStat = true;
        }
