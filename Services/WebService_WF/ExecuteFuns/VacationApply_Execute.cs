...
namespace WebService_WF.ExecuteFuns
{
    ...
    /// 休假申请流程
    /// </summary>
    public class VacationApply_Execute
    {
        ...
        public VacationApply_Execute(WorkflowDetail obj)
        {
            workflowDetail = obj;

            string confirmStepName = WfSectionGroup.VacationApply["Confirm"].ToString();
        ...
        public void ConfirmStep_Approve()
        {
            ...
            {
                LogRecord.WriteLog(string.Format("流程“{0}(受理号：{1})”执行步骤“{2}”后调用服务失败：获取休假申请单ID失败", workflowDetail.WorkflowName, workflowDetail.SheetId, workflowDetail.PreActivities[0].Name), LogResult.fail);
                return;
            }

            HRVacationApply curApply = _applyService.FindById(dataID);
            if (curApply == null || curApply.ApplyId == Guid.Parse("00000000-0000-0000-0000-000000000000"))
            {
                LogRecord.WriteLog(string.Format("流程“{0}(受理号：{1})”执行步骤“{2}”后调用服务失败：查询不到对应休假申请单信息", workflowDetail.WorkflowName, workflowDetail.SheetId, workflowDetail.PreActivities[0].Name), LogResult.fail);
                return;
            }
